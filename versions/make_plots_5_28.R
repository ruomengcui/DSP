...
plot.mc<-function(trials, graph.name){
  d=ldply(trials, function(x){
    load(paste(varSave, 'simPanel_', x, '.txt', sep=""))
    load(paste(varSave, 'simEst_', x, '.txt', sep=""))
    p=unique(d[,names(d)=="cid"| substr(names(d), 1, 5)=="alpha"|substr(names(d), 1, 4)=="beta"])
    count.alpha=sum(substr(names(p), 1, 5)=="alpha")
    count.beta=sum(substr(names(p), 1, 4)=="beta")
    e=ldply(o, function(x){
      ...
      colnames(o)<-c("samp", "cid", paste("alpha_", seq(count.alpha), sep=""), paste("beta_", seq(count.beta), sep=""))
      o
    })
    s=ddply(e[e$samp>0,-which(names(e)=="samp")], "cid", function(x) sd(x)[2:dim(x)[2]])
    ...
    a$var<-factor(a$var, levels=c("a1", "a2", "b1", "b2"))
    levels(a$var)<-c(expression(alpha[0]),expression(alpha[1]),expression(beta[0]),expression(beta[1]))
    ...
    a$l=a$e-1.96*a$s
    a$c=a$p<a$u & a$p>a$l
    a$sigma=paste(expression(sigma^2), "==", x, sep="")
    a
  })
	ggplot(d,aes(y=e,x=p,colour=c))+geom_point()+geom_errorbar(aes(max=u,ymin=l))+geom_abline(intercept=0,slope=1)+facet_grid(var~sigma,labeller=label_parsed)+scale_colour_grey(end=0.7,start=0)+labs(x="Parameter", y="Estimate")+coord_cartesian(ylim=c(-.05,4.3), xlim=c(-.05, 2.05))+scale_x_continuous(breaks=seq(0, 2, .5))+scale_y_continuous(breaks=seq(0, 4, 1))+opts(panel.background=theme_rect(colour="lightgrey"), panel.background = theme_rect(colour=NA), legend.position = "none", strip.text.y=theme_text(size=12), strip.text.x=theme_text(size=12), axis.title.x=theme_text(size = 14), axis.title.y=theme_text(size = 14, angle=90), axis.text.y=theme_text(size=11), axis.text.x=theme_text(size = 11))
