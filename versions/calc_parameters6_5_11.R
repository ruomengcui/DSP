calc.firm<-function(id){
	...
	for(i in 1:3) Lambda[,,i]=est.Lambda(Eo, E, A[,,i])
	...
	print(id)
	list(id=id, A=A, S=S, Lambda=Lambda, W=W, gamma=gamma, theta=theta, Omega=Omega, Xi=Xi, Psi=Psi, m=D2$m[r2], sic=sic, biz=biz, objectives=c(gmm(A[,,2]), gmm(A[,,3])))
}
