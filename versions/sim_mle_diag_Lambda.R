...
load.est.group<-function(my.samp, gamma.scaler, group, k, sim.count){
	est.group<-function(group.id){
		...
		optim(c(log(rep(.3, k[2])), c(diag(k[2])[lower.tri(diag(k[2]), diag=TRUE)]), rep(log(30), k[1])), mle.objective, gr=NULL, method="L-BFGS-B", lower=c(rep(-11, k[2]+k[2]*(k[2]+1)/2), rep(-Inf, k[1])), upper=c(rep(11, k[2]+k[2]*(k[2]+1)/2), rep(Inf, k[1])), control=list(maxit=50000))
	} 
}
...
load.mle.objective<-function(draws, demand, order, gs, k, firm.count, sim.count){
	mle.objective<-function(param){
		print(param[1:9])
		p1=k[2]; p2=k[2]*(k[2]+1)/2; p3=k[1]
		theta.LD=diag(k[2])
		theta.LD[lower.tri(diag(k[2]), diag=TRUE)]<-param[seq(p2)+p1]
		Lambda.LD=diag(exp(param[seq(p3)+p1+p2]))
