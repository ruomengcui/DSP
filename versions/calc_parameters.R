calc.firm<-function(id){
	...
	print(id)
	list(id=id, A=A, S=S, Lambda=Lambda, W=W, gamma=gamma, theta=theta, Omega=Omega, Xi=Xi, Psi=Psi, m=D2$m[r2], sic=sic, biz=biz, objectives=objectives, r=substr(id[2], nchar(id[2]), nchar(id[2])))
}
...
est.TN<-function(gmm, k){
	objective.TN<-function(log.gamma){
		...
		gmm(getA(c(exp(log.gamma), rep(0, k-1)), 0))+norm.penalty
	}	
	count=0
	while(count<5){
